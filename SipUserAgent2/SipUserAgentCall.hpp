...
bool CSipUserAgent::StartCall( const char * pszFrom, const char * pszTo, CSipCallRtp * pclsRtp, CSipCallRoute * pclsRoute, std::string & strCallId )
{
	...
	pclsDialog->m_strFromId = pszFrom;
	pclsDialog->m_strToId = pszTo;

	pclsDialog->SetLocalRtp( pclsRtp );

	pclsDialog->m_strContactIp = pclsRoute->m_strDestIp;
	pclsDialog->m_iContactPort = pclsRoute->m_iDestPort;
	pclsDialog->m_eTransport = pclsRoute->m_eTransport;
	pclsDialog->m_b100rel = pclsRoute->m_b100rel;

	if( SendInvite( pclsDialog ) == false ) return false;
	strCallId = pclsDialog->m_strCallId;
